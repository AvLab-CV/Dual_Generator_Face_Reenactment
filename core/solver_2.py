...
class Solver(nn.Module):
    def __init__(self, args):
        ...
        if args.mode == 'train':
            ...
            self.ckptios = [CheckpointIO(ospj(args.checkpoint_dir, '{:06d}_nets_ema.ckpt'.format(100000)), **self.nets_ema)]
    ...
    def train(self, loaders):
        ...
        for i in range(args.resume_iter, args.total_iters):
            ...
            g_loss, g_losses_latent = compute_g_loss(
                nets, args, x_real, y_org, y_trg, arcface, conf, z_trgs=[z_trg, z_trg2], masks=masks)
            self._reset_grad()
            g_loss.backward()
...
def compute_g_loss(nets, args, x_real, y_org, y_trg, arcface, conf, z_trgs=None, x_refs=None, masks=None):
...
def compute_g_loss_2(nets, args, x_real, y_org, y_trg, arcface, conf, z_trgs=None, x_refs=None, masks=None):
    assert (z_trgs is None) != (x_refs is None)
    if z_trgs is not None:
        z_trg, z_trg2 = z_trgs
    if x_refs is not None:
    ...
    loss_id_fake1 = arcface.extract_fea(args, conf, x_real, x_fake, False)
    loss_id_fake2 = arcface.extract_fea(args, conf, x_real, x_rec, False)
    loss_id_fake3 = arcface.extract_fea(args, conf, x_real, x_fake, False)
    loss_id = loss_id_fake1 + loss_id_fake2 + loss_id_fake3


    loss = loss_adv + args.lambda_sty * loss_sty \
        + args.lambda_ds * loss_ds + args.lambda_cyc * loss_cyc + args.lambda_id * loss_id
    return loss, Munch(adv=loss_adv.item(),
                       sty=loss_sty.item(),
                       ds=loss_ds.item(),
                       cyc=loss_cyc.item(),
